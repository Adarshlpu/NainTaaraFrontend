...
const Footer = () => {
  ...
  const productLinks = ["Features", "Games", "Rewards", "Eye Theory"];
  const companyLinks = ["About", "Blog", "Contact", "Careers"];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
            ...
            <ul className="space-y-3 sm:space-y-4">
              {productLinks.map((item) => (
                <motion.li
                  key={item}
                  whileHover={{ x: 8 }}
                  className="text-gray-600 hover:text-[#ff7a00] transition-all duration-300 cursor-pointer text-sm sm:text-base font-medium"
                >
                  {item}
                </motion.li>
              ))}
          ...
          <motion.div variants={itemVariants}>
            ...
            {/* CTA Button */}
            <motion.button
              whileHover={{ scale: 1.05, boxShadow: "0 10px 30px rgba(255, 122, 0, 0.3)" }}
              whileTap={{ scale: 0.95 }}
              className="w-full sm:w-auto bg-gradient-to-r from-[#ff7a00] to-orange-600 text-white px-6 sm:px-7 py-3 rounded-full shadow-lg hover:shadow-orange-300 transition-all duration-300 font-semibold text-sm sm:text-base"
            >
              Start Playing →
            </motion.button>
          </motion.div>
        ...
        >
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
            ...
            >
              © 2026 Naintaara. All rights reserved.
            </motion.p>
